...
var (
	...
	testStressConnectionsCounter = promauto.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "The number of connections successfully opened towards the external target",
	}, []string{"operation"})

	testStressConnectionLatency = promauto.NewHistogramVec(prometheus.HistogramOpts{
	...
	}, []string{"round"} /* split the metric in rounds of 5K connections, to check degradation */)
)
...
func stressExternalTarget(
	...
	for errcnt < maxerrs {
		...
		round := strconv.FormatInt(int64(count/roundSize), 10)
		testStressConnectionsCounter.WithLabelValues("open").Inc()
		testStressConnectionLatency.WithLabelValues(round).Observe(elapsed)
